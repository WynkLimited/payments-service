...
public class PaymentRecurringSchedulingKafkaConsumer extends AbstractKafkaEventConsumer<String, PaymentRecurringSchedulingMessage> {
    ...
    @Autowired
    private KafkaRetryHandlerService<String, PaymentRecurringSchedulingMessage> kafkaRetryHandlerService;

    public PaymentRecurringSchedulingKafkaConsumer (PaymentManager paymentManager, KafkaListenerEndpointRegistry endpointRegistry) {
        super();
    ...
    @KafkaListener(id = "paymentRecurringSchedulingMessageListener", topics = "${wynk.kafka.consumers.listenerFactory.paymentRecurringScheduling[0].factoryDetails.topic}", containerFactory = "${wynk.kafka.consumers.listenerFactory.paymentRecurringScheduling[0].name}")
    protected void listenPaymentRecurringSchedulingMessage(@Header(value = StreamConstant.MESSAGE_LAST_ATTEMPTED_SEQUENCE, required = false) String lastAttemptedSequence,
                                                           ...
                                                           ConsumerRecord<String, PaymentRecurringSchedulingMessage> consumerRecord) {
        try {
            ...
            if (!(e instanceof WynkRuntimeException)) {
                log.error(StreamMarker.KAFKA_POLLING_CONSUMPTION_ERROR, "Something went wrong while processing message {} for kafka consumer : {}", consumerRecord.value(), ", PaymentRecurringSchedulingMessage - ", e);
            }
        }
    }
