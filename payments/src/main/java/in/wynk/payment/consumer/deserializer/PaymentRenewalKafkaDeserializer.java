...
@Slf4j
public class PaymentRenewalKafkaDeserializer implements Deserializer<PaymentRenewalMessage> {

    private final ObjectMapper objectMapper = new ObjectMapper();

    @Override
    public PaymentRenewalMessage deserialize(String topic, byte[] bytes) {
        try {
            return objectMapper.readValue(bytes, PaymentRenewalMessage.class);
        } catch (Exception e) {
