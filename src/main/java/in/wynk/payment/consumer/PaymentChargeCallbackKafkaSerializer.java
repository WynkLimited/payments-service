 ...
 */
@Slf4j
public class PaymentChargeCallbackKafkaSerializer implements Deserializer<PaymentChargeKafkaMessage> {
    private final ObjectMapper objectMapper = new ObjectMapper();

    @Override
    public PaymentChargeKafkaMessage deserialize(String topic, byte[] bytes) {
        try {
            String message = objectMapper.readValue(bytes, String.class);
            log.info("response received for invoice callback - " + message);
            return objectMapper.readValue(message, PaymentChargeKafkaMessage.class);
        } catch (Exception e) {
