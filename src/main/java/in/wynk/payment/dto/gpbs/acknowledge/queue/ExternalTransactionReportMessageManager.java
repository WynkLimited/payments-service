...
@DelayedKafkaEvent(topic = "${wynk.kafka.consumers.listenerFactory.externalTransactionReport[0].factoryDetails.topic}", producerType = ProducerType.QUARTZ_MESSAGE_PRODUCER, quartz = @DelayedKafkaEvent.QuartzConfiguration(expression = "T(java.util.Arrays).asList(60, 60, 60, 60, 120, 300, 300, 300, 890, 890, 2400, 3600, 79200, 172800, 179800).get(#n)", publishUntil = 1, publishUntilUnit = TimeUnit.DAYS))
