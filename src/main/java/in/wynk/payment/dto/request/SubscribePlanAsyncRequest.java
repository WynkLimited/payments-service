...
public class SubscribePlanAsyncRequest extends AbstractSubscribePlanRequest {
    ...
    public static SubscribePlanAsyncRequest from(AsyncTransactionRevisionRequest request) {
        ...
        return builder.uid(transaction.getUid()).msisdn(transaction.getMsisdn()).planId(transaction.getPlanId()).transactionId(transaction.getIdStr()).paymentEvent(transaction.getType()).transactionStatus(transaction.getStatus()).triggerDataRequest(getTriggerData()).paymentCode(transaction.getPaymentChannel()).build();
    }
