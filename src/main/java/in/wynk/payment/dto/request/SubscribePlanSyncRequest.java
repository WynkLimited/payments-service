...
public class SubscribePlanSyncRequest extends AbstractSubscribePlanRequest {
    ...
    public static SubscribePlanSyncRequest from(SyncTransactionRevisionRequest request) {
        ...
        return builder.uid(transaction.getUid()).msisdn(transaction.getMsisdn()).planId(transaction.getPlanId()).transactionId(transaction.getIdStr()).paymentEvent(transaction.getType()).transactionStatus(transaction.getStatus()).paymentCode(transaction.getPaymentChannel()).os(TransactionContext.getPurchaseDetails().map(IPurchaseDetails::getAppDetails).map(IAppDetails::getOs).orElse(UNKNOWN)).build();
    }
