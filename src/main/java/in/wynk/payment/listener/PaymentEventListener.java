...
public class PaymentEventListener {
    ...
    public void onTransactionSnapshotEvent (TransactionSnapshotEvent event) {
        ...
        if(event.getTransaction().getStatus() == TransactionStatus.SUCCESS && event.getTransaction().getPaymentChannel().getCode()== PAYU  )
        {
            final PayUTdrResponse Tdr = BeanLocatorFactory.getBean(event.getTransaction().getPaymentChannel().getCode(),IMerchantTDRService.class).getTDR(TRANSACTION_ID);
            AnalyticService.update(Tdr);
        }
