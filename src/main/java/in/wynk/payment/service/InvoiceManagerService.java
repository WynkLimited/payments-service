...
public class InvoiceManagerService implements InvoiceManager {
    ...
    public void generate(GenerateInvoiceRequest request) {
        try{
            ...
            retryInvoiceGeneration(request.getMsisdn(), request.getClientAlias(), request.getTxnId(), request.getType());
    ...
    public void processCallback(InvoiceCallbackRequest request) {
        try{
            ...
            if(request.getStatus().equalsIgnoreCase(InvoiceState.FAILED.name())){
                retryInvoiceGeneration(transaction.getMsisdn(), transaction.getClientAlias(), invoice.getTransactionId(), request.getType());
            }
        } catch(Exception ex){
    ...
    private void retryInvoiceGeneration (String msisdn, String clientAlias, String txnId, String type) {
