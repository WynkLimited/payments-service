...
import in.wynk.payment.dto.manager.ChargingResponseWrapper;
...
public class PaymentGatewayManager implements IPaymentChargingService<ChargingResponseWrapper<?>, AbstractChargingRequest<?>> {
    ...
    public ChargingResponseWrapper<?> charge(AbstractChargingRequest<?> request) {
        ...
        final IPaymentChargingService<AbstractPaymentChargingResponse, AbstractChargingRequest<?>> chargingService = BeanLocatorFactory.getBean(pg.getCode().concat(VERSION_2), new ParameterizedTypeReference<IPaymentChargingService<AbstractPaymentChargingResponse, AbstractChargingRequest<?>>>() {
        });
        try {
